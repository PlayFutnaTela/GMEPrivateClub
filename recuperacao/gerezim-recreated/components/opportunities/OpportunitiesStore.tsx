            ...
            <Card key={item.id} className="overflow-hidden flex flex-col h-full">
              ...
              <CardContent className="flex-1">
                <div className="text-2xl font-bold mb-2 text-gold-500">
                  {'currency' in item && item.currency && item.currency !== 'BRL' ? (
                    <div className="flex flex-col">
                      <span>
                        {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: item.currency }).format('price' in item ? item.price : Number(item.value))}
                      </span>
                      <span className="text-[10px] text-slate-500 font-semibold uppercase mt-1">PRODUTO EM {item.currency === 'USD' ? 'DÓLAR' : item.currency === 'EUR' ? 'EURO' : item.currency}</span>
                    </div>
                  ) : (
                    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format('price' in item ? item.price : Number(item.value))
                  )}
                </div>
                <p className="text-sm text-muted-foreground line-clamp-2 mb-3">{item.description}</p>
                {'location' in item && item.location && (
                  <p className="text-xs text-muted-foreground flex items-center gap-1 mb-2"><MapPin className="h-3 w-3" /> {item.location}</p>
                )}
                ...
                {'stock' in item && (<p className="text-xs text-muted-foreground mt-2">Estoque: {item.stock}</p>)}
              </CardContent>
              ...
              <CardFooter>
                <div className="flex w-full gap-2">
                  ...
                  <Button onClick={() => handleWhatsApp(item)} className="flex-1 bg-gold-500 text-white hover:bg-gold-600"><ShoppingCart className="h-4 w-4 mr-2" /> {'images' in item ? 'Comprar' : 'Negociar'}</Button>
                </div>
              </CardFooter>
            </Card>
          ))}
          ...
          {filteredItems.map((item) => (
            <Card key={item.id}>
              <div className="flex gap-6 p-4">
                {'images' in item && item.images?.length > 0 ? (
                  <img src={item.images[0]} alt={item.title} className="w-32 h-32 object-cover rounded-lg flex-shrink-0" />
                ) : 'photos' in item && item.photos?.length > 0 ? (
                  <img src={item.photos[0]} alt={item.title} className="w-32 h-32 object-cover rounded-lg flex-shrink-0" />
                ) : (
                  <div className="bg-gray-200 border-2 border-dashed rounded-lg w-32 h-32 flex-shrink-0 flex items-center justify-center"><span className="text-gray-500 text-xs">Sem imagem</span></div>
                )}
                ...
                <div className="flex-1">
                  <div className="flex justify-between">
                    <div>
                      ...
                      {'subtitle' in item && item.subtitle && (<p className="text-sm text-muted-foreground">{item.subtitle}</p>)}
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="icon" onClick={() => toggleFavorite(item.id)}>
                        <Heart className={`h-5 w-5 ${favorites.has(item.id) ? 'fill-red-500 text-red-500' : 'text-gray-500'}`} />
                      </Button>
                      <Badge variant={'status' in item && item.status === 'vendido' ? 'secondary' : 'default'}>{item.category.toUpperCase()}</Badge>
                      <Badge variant="outline" className="text-xs">{'images' in item ? 'Produto' : 'Oportunidade'}</Badge>
                    </div>
                  </div>

                  <div className="flex items-center gap-4 mt-2">
                    <div className="text-2xl font-bold text-gold-500">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format('price' in item ? item.price : Number(item.value))}</div>
                    <Badge variant={'status' in item && item.status === 'em_negociacao' ? 'secondary' : 'default'}>{'status' in item && item.status === 'em_negociacao' ? 'Em negociação' : 'status' in item && item.status === 'active' ? 'Ativo' : 'status' in item ? item.status.charAt(0).toUpperCase() + item.status.slice(1) : 'Novo'}</Badge>
                  </div>

                  <p className="text-muted-foreground mt-2 line-clamp-2">{item.description}</p>

                  <div className="flex items-center gap-4 mt-3">
                    {'location' in item && item.location && (<p className="text-sm text-muted-foreground flex items-center gap-1"><MapPin className="h-4 w-4" /> {item.location}</p>)}
                    {'tags' in item && item.tags && item.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {item.tags.slice(0, 3).map((tag, index) => (<Badge key={index} variant="secondary" className="text-xs">{tag}</Badge>))}
                        {item.tags.length > 3 && (<Badge variant="secondary" className="text-xs">+{item.tags.length - 3}</Badge>)}
                      </div>
                    )}
                    {'stock' in item && <p className="text-sm text-muted-foreground">Estoque: {item.stock}</p>}
                    <div className="flex items-center"><Star className="h-4 w-4 fill-yellow-400 text-yellow-400" /><span className="text-sm ml-1">4.8 (120)</span></div>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between">
                  <div className="flex gap-2">
                    ...
                    <Button onClick={() => handleWhatsApp(item)} className="bg-gold-500 text-white hover:bg-gold-600"><ShoppingCart className="h-4 w-4 mr-2" /> {'images' in item ? 'Comprar' : 'Negociar'}</Button>
                  </div>
                </div>
              </div>
            </Card>
